# RTS Plan Archive — completed work, with its evidence

> **This is history, not a roadmap. It is frozen.**
> The live plan is **[MASTER_PLAN.md](MASTER_PLAN.md)** — that is the single
> source of truth for what's being worked on. Split out 2026-07-17, when the
> plan had reached 1,898 lines / 134 done vs 15 open (~90 % archive) and cost a
> quarter of a context window to read.
>
> **Why this file exists at all** (rather than `git log`): the completed entries
> below carry *rationale and measured validation data* that git history does not
> surface — most importantly the **failed experiments**. "Rusher attack threshold
> 4 was tried and lost same-seed validation, reverted to 6"; "ram hp 280 made
> sieges fail, 300/armor-0/damage-85-115 is the keeper"; "a denser ring was
> measurably not enough, 465/465 failures had a slot the lattice found". Those
> are the entries that stop a future session from confidently redoing a thing
> that already didn't work. Read this file before re-tuning anything it touches.
>
> **Section numbers are preserved** (§7.2, §8.10, §8.11 …) so references from
> commit messages, code comments, and CLAUDE.md still resolve here.
>
> ⚠ **Unticked `- [ ]` items below are stale** — they were carried forward into
> MASTER_PLAN.md at the split. Do not work from them; work from the live plan.

---

# RTS Master Plan — Performance Foundation & Single-Player Depth *(as of the 2026-07-17 split)*
...
> ⚠ **SUPERSEDED 2026-07-17 — see MASTER_PLAN §8.2.2.** The tile anatomy above
> ("icon + wrapped name + compact cost") is **no longer the target**. Shipped, it
> proved unreadable at the default 1920×1080 (the sidebar never scaled: 14–16 px
> fonts in a hardcoded 200 px column, ~13 px of tile slack, an 11 px line step
> against an 11 px font). The replacement is **icon-first**: the name leaves the
> tile and the hover tooltip carries name/duration/description/counters, while
> **cost stays on the tile as icons** — which is the one convention above that this
> section's own research got right and that survives verbatim (cost-on-tile is
> universal; it's what makes affordability scannable across the grid).
> Everything else here — the sidebar-vs-bottom-bar rationale, always-visible
> category tabs, grid hotkeys, batch production, the global queue — still stands.
...
### 8.13 Bug batch 2026-07-17 round 2 — user-reported from live play *(landed 2026-07-17)*

All three root-caused with file:line evidence before any code was written. Two
were shipped features certified by tests that structurally could not observe
the failure — so each fix closed its test gap first. (§8.13.4, the hex
round-trip, stays open in the live plan — deliberately, the fix is more
dangerous than the bug.)

- [x] **8.13.1 Workers don't continue to a nearby node on depletion.** §8.11's
  continuation was real but dead code in the dominant path: a node running dry
  almost always leaves the worker CARRYING (the gather tick returns `"full"`,
  not `"depleted"`), and same-frame `_cleanup_destroyed_objects` cancelled the
  task, nulled both gather refs, and re-issued a bare dropoff — laundering
  `kind="gather"` into `kind="dropoff"`/`resource=None`, so delivery completed
  to idle. The AI's idle-scan re-tasked its own workers every tick, which is
  why only *human* workers showed it. **Fix: cleanup skips workers the task
  system `owns()`** — the task system already handles depletion end-to-end.
  Siblings hardened in passing: a gather node that vanishes mid-walk continues
  instead of failing to idle, and `_continue_or_complete` delivers carried
  cargo when no continuation node is in range. **Test gap closed first:** the
  suite's `tick()` helper never ran cleanup, so the §8.11 test passed against
  a loop the real game never runs. Cleanup added to the helper →
  `test_worker_continues_to_nearby_node_when_depleted` failed exactly as
  predicted → fix → green. New tests: human-flagged dominant-path
  continuation; node-vanished-mid-walk.

- [x] **8.13.2 Floating text renders through unexplored fog.**
  `draw_notifications` was the only floating-UI draw with no fog gate (the
  exact sibling of §9's health-bar leak — same class, one call missed). Floats
  draw above the fog overlay by design, so they must self-censor. Both traps
  the naive fix would hit were handled: notifications stored no owner/source
  (own floats would have vanished at fog edges) → **owner + true origin
  captured at spawn**; and the stored x/y is offset at spawn and drifts
  ~30 px/s upward → **visibility judged per-frame against the fixed origin**
  (fog rebuilds at 5 Hz under a 0.8–1.4 s float, so spawn-time gating alone is
  wrong in both directions). Bonus: floats no viewer can currently see are not
  spawned at all — every AI's farms fired one per cycle, and fogged AI-vs-AI
  battles fired the most of all. Spectator `reveal_display` and own-player
  floats verified unaffected. Tests: spawn gate, render gate incl. spectator,
  origin-vs-drift (`test_ui_bugfixes.py`).

- [x] **8.13.3 Units get stuck near water — Phase 3 reopened.** Four
  compounding causes, all landed 2026-07-17:
  1. **No terrain sliding** (the atomic reject killed the along-shore
     component) → axis-decomposed sliding at FULL step length. Full length
     matters: the raw projected component can be sub-epsilon on a
     near-horizontal path leg (0.09 px/frame reads as "blocked" against the
     0.1 px moved-threshold and freezes — found live in the crowd test).
  2. **Steering probed a center point at radius 0** while the mover rejects at
     ±radius — crowds livelocked re-picking the rejected slot via the +0.08
     hysteresis (lone units fine, crowds broke, matching the report) → slots
     validated with the mover's own full-radius check, hysteresis cleared on a
     hard block.
  3. **Watchdog `_is_safe` center-point probe** was strictly weaker than the
     movement gate — shore-wedged units (center on land, body in water) judged
     "already safe", never nudged, idle forever once resumes ran out. The old
     CLAUDE.md claim "the watchdog recovers stuck units" was false at water →
     full-radius test; the claim is true again.
  4. **The correct rescue was dead code** — `_ensure_unit_on_walkable_terrain`
     had zero callers → wired into the terrain gate: a unit already
     overlapping water snaps to the nearest clear spot instead of the old
     escape hatch that let it move anywhere, *including deeper in*. New
     `terrain_rescues` perf counter for observability.
  Plus two adjacent wedge-creators: production spawn probes now test the full
  16 px spawn radius (a barracks near water spawned units body-into the
  shore), and the blocked-unit handler no longer discards lakeshore detours —
  waypoint skips are refused when the skipped-to leg crosses blocked terrain
  (`segment_clear`), and a unit stuck on its LAST waypoint re-plans instead of
  the old silent no-op that re-blocked every 0.5 s forever. Both handler bugs
  were caught live: 2 of 20 crowd units pinned at the lake's west face with a
  final waypoint aimed straight across the bay, handler no-opping in a loop.
  **Evidence:** new `tests/test_shoreline_movement.py` (real collision +
  movement + pathfinding over a lake map): a 20-unit crowd ordered across the
  lake's latitude arrives 20/20 with nobody wedged; shoreline slide, wedge
  rescue, watchdog full-radius, and spawn-probe each asserted at unit level.
  Full suite 385 green. Benchmark (120 s, 4p, seed 12345): avg 3.0 / p95 12.0
  / teleports 0 / recoveries 0 / terrain_rescues 1. ⚠ frame_max ~84 ms — 
  measured **pre-existing** (clean-HEAD `git archive` baseline: 93.5 ms, same
  seed/duration; §6 recorded 23 ms on 07-13, so a commit between 07-13 and
  b33e701 added an early-game hitch — flagged as its own investigation, not
  this batch).
  **Deliberately deferred:** planner-graph erosion near terrain (changes every
  path; gate on a §6 re-profile — steering fixes may have moved the numbers);
  `has_line_of_sight` half-width (amplifier only); and the
  `find_safe_spawn_position` corner-return, folded into §8.13.4 — its 3×3
