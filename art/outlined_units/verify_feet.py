...
anchors=game.rendering_system._ground_anchors
captures=[]
for active in [False,True]:
    game.rendering_system._ground_anchors=anchors if active else {}
...
for i,label in enumerate(['Before: radius estimate','Fixed: Blender foot anchor']):
    draw.text((i*320+12,12),label,font=font,fill='#efdfbe')
    board.paste(captures[i],(i*320+10,49))
board.save(ROOT/'art/outlined_units/foot-alignment.png')
pygame.quit()
