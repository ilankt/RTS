...
@pytest.mark.parametrize('name',['worker','warrior','archer','spearman','healer','cavalry'])
...
def test_marker_and_shadow_share_blender_ground_point(name,zoom):
    ...
    anchor=record['ground_anchor']
    scale=record.get('render_scale',1)
    renderer=RenderingSystem.__new__(RenderingSystem)
    renderer._render_scales={name:scale}
    ...
    assert point[1]==pytest.approx(200*zoom-31+(anchor[1]-.5)*64*scale*zoom)
